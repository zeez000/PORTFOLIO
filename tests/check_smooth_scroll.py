...
try:
    ...
    with sync_playwright() as p:
        ...
        for name, w, h, touch in [('small-mobile',320,740,True),('desktop',1440,900,False),('wide',1920,1080,False),('tablet',768,1024,False),('mobile',390,844,True)]:
            ...
            check(name + ': original reveal still works', page.locator('#about .reveal').evaluate('(e)=>e.classList.contains("visible")'))
            ...
            check(name + ': text flow initialized or safely reduced', page.evaluate("window.textFlow && window.textFlow.ready"))
        ...
        navigate(page, 'projects')
        check('Blocked CDN: navigation and reveals work', page.evaluate('location.hash') == '#projects' and page.locator('#projects .reveal').evaluate('(e)=>e.classList.contains("visible")'))
